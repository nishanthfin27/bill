...
const UserDashboard = (props) => {
    ...
    return (
        <div><br /><br /><br />
            ...
            <h1>Recently Added Customers</h1><br />
            <div className='horizontalAlign'>
                {recentFiveCusts.map((cust) => {
                if(cust) {
                    return (
                        <div key={cust._id}>
            ...
            <h1>Recently Added Products</h1><br />
            <div className='horizontalAlign'>
            {recentFiveProds.map((prod) => {
                if(prod) {
                    return (
                        <div key={prod._id}>
                            <Card aria-label='Basic vertical card'>
                                ...
                                <Text>Purchase details</Text>
                                    {
                                        bill.lineItems.map((item,i) => {
                                            return <li key={i}><Text>{displayname(item.product)} -{item.price}rs * {item.quantity} = {item.subTotal}</Text></li>
                                        })
                                    }
